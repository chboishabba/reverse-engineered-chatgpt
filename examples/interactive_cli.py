...
The script offers a single entrypoint that validates cached session tokens
before guiding the user through retrieving the
``__Secure-next-auth.session-token`` cookie.  Once authenticated it opens a
streaming chat loop using :class:`~re_gpt.sync_chatgpt.SyncChatGPT`.
"""
